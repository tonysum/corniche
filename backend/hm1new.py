...
【4️⃣ 补仓机制】
  • 启用控制：默认启用，可通过 --disable-add-position 禁用
  • 触发条件：价格从平均成本下跌18%（可通过 --add-position-trigger-pct 自定义）
  • 补仓次数：最多补仓1次
  • 补仓金额：当前资金 × 5%（默认，可通过 --add-position-size-ratio 自定义）
  • 首次建仓：当前资金 × 5%
  • 成本重算：补仓后重新计算平均成本
...
基本参数：
  ...
  --threshold          买量暴涨阈值倍数（默认: 13.1倍）
  --max-multiple       买量暴涨倍数上限（默认: 3.0，例如10表示接受2-10倍信号）

动态止盈参数：
  --dynamic-tp-boost              动态止盈加成幅度（覆盖分档配置，例如 0.05 表示统一+5%）
  --dynamic-tp-lookback-minutes   动态止盈强势判定窗口长度（分钟，默认720=12小时）
  --dynamic-tp-close-up-pct       动态止盈强势判定涨幅阈值（默认0.025=+2.5%）

补仓控制参数：
  --disable-add-position           禁用补仓功能（默认启用）
  --add-position-trigger-pct      补仓触发比例（默认-0.18，即-18%）
  --add-position-size-ratio       补仓占资金比例（默认0.05，即5%）

使用示例：
  # 基本回测
  python hm1.py --start-time 2025-11-01 --end-time 2026-01-10

  # 禁用补仓
  python hm1.py --start-time 2025-11-01 --end-time 2026-01-10 --disable-add-position

  # 自定义补仓参数（下跌15%补仓，使用8%资金）
  python hm1.py --start-time 2025-11-01 --end-time 2026-01-10 \\
    --add-position-trigger-pct -0.15 --add-position-size-ratio 0.08

  # 接受2-10倍信号，统一动态止盈加成5%
  python hm1.py --start-time 2025-11-01 --end-time 2026-01-10 \\
    --max-multiple 10 --dynamic-tp-boost 0.05
...
创建时间：2026-01-11
最后更新：2026-01-16
版本：v2.1（添加补仓控制参数，支持自定义补仓触发比例和金额比例）
"""
...
class BuySurgeBacktest:
    ...
    def __init__(self):
        # PostgreSQL 不需要 PRAGMA 设置，直接使用 engine
        pass
        ...
        self.take_profit_pct = 0.11  # 止盈比例 (11%)
        ...
        # 🚀 性能优化：缓存小时K线数据（按日期范围，避免重复查询）
        # 格式：{(symbol, start_date, end_date): DataFrame}
        self._hourly_kline_cache = {}
        self._cache_enabled = True  # 是否启用缓存（默认启用）
        
        self.enable_add_position = True  # 是否启用补仓功能（默认启用）
    ...
    def calculate_dynamic_take_profit(
        ...
        try:
            ...
            if current_datetime >= window_2h_end:
                # 2小时已过，检查5分钟K线表现
                try:
                    # 🔧 使用统一的5分钟数据查询方法（使用trade_date字段）
                    closes = self.get_5m_closes_in_window(symbol, entry_datetime, window_2h_end)
                    ...
                    if len(closes) >= 24:  # 确保有足够的K线数据
                        ...
                        if len(closes) > 0:
                            logging.warning(
                                f"⚠️ {symbol} 5分钟数据不足：需要24根（2小时），实际{len(closes)}根，"
                                f"跳过2小时强势币判定，使用默认止盈{self.take_profit_pct*100:.1f}%"
                            )
                        else:
                            logging.debug(f"⚠️ {symbol} 无5分钟数据，跳过2小时强势币判定")
                except Exception as e:
                    logging.warning(f"⚠️ 查询{symbol} 2小时5分钟K线数据失败: {e}")
    ...
    def get_daily_1hour_surge_signals(self, check_date: str) -> List[Dict]:
        ...
        try:
            ...
            # 🔧 性能优化：限制扫描的交易对数量，避免过多导致卡死
            max_symbols_to_scan = 5000  # 最多扫描5000个交易对
            symbols_to_scan = all_symbols[:max_symbols_to_scan] if len(all_symbols) > max_symbols_to_scan else all_symbols
            
            # 🚀 性能优化：批量查询昨日数据（减少数据库往返）
            yesterday_volumes = {}  # {symbol: volume}
            for symbol in symbols_to_scan:
                try:
                    daily_table = f'K1d{symbol}'
                    safe_table_name = f'"{daily_table}"'
            ...
            for idx, symbol in enumerate(symbols_to_scan, 1):
                ...
                try:
                    # 🚀 步骤1：从缓存获取昨日日K线总买量
                    if symbol not in yesterday_volumes:
                        continue
                    
                    yesterday_daily_volume = yesterday_volumes[symbol]
                    # 🔧 关键修复：计算昨日平均小时买量（1天 = 24小时）
                    yesterday_avg_hour_volume = yesterday_daily_volume / 24.0
                    
                    # 🚀 步骤2：获取今日所有小时K线（使用缓存优化）
                    hourly_df = self.get_hourly_kline_data(symbol, start_date=check_date, end_date=check_date)
                    if hourly_df.empty:
                        continue
                    
                    # 🚀 步骤3：找到第一个满足条件的小时
                    for _, row in hourly_df.iterrows():
                        hour_time = row['trade_date']
                        hour_volume = row.get('active_buy_volume')
                        hour_price = row.get('close')
                        ...
                        if surge_ratio >= threshold and surge_ratio <= self.buy_surge_max:
                            ...
                            logging.debug(f"🔥 发现信号: {symbol} @{signal_datetime.strftime('%H:00')} 倍数{surge_ratio:.2f}x 价格{hour_price:.6f}")
                            break  # 只记录第一个满足条件的小时
                ...
                # 每扫描500个交易对显示一次进度（减少日志输出频率）
                if idx % 500 == 0 or idx == len(symbols_to_scan):
                    progress_pct = idx * 100 // len(symbols_to_scan) if len(symbols_to_scan) > 0 else 0
                    logging.info(f"  扫描进度: {idx}/{len(symbols_to_scan)} ({progress_pct}%) | 已找到信号: {len(signals)} 个")
    ...
    def get_hourly_kline_data(self, symbol: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
        ...
        if self._cache_enabled:
            ...
            if cache_key in self._hourly_kline_cache:
        ...
        try:
            ...
            if self._cache_enabled:
                cache_key = (symbol, start_date, end_date)
                self._hourly_kline_cache[cache_key] = df.copy()
    ...
    def check_exit_conditions(self, position: Dict, current_price: float, current_date: str) -> bool:
        ...
        try:
            ...
            if not hold_period_data.empty:
                # 🔧 性能优化：限制检查的小时数，避免数据量过大时卡死
                max_hours_to_check = 1000  # 最多检查1000小时（约42天）
                rows_to_check = hold_period_data.tail(max_hours_to_check) if len(hold_period_data) > max_hours_to_check else hold_period_data
    ...
    def exit_position(self, position: Dict, exit_price: float, exit_date: str, exit_reason: str):
        ...
        try:
            # 🔧 关键修复：防止重复平仓
            # 如果持仓已经有exit_date，说明已经平仓了，直接返回
            if position.get('exit_date'):
                logging.warning(f"⚠️ 持仓 {position.get('symbol', 'unknown')} 已经平仓，跳过重复平仓操作")
                return
            ...
            # 🆕 记录平仓前的资金（用于计算平仓后的余额）
            capital_before_exit = self.capital
            # 平仓时返还本金+盈亏
            self.capital += position_value + pnl
            
            # 🆕 修复：基于该笔交易的建仓后余额计算平仓后余额，而不是使用全局self.capital
            # 因为self.capital可能已经被其他并发交易修改了
            capital_after_entry = position.get('capital_after_entry')
            add_position_total_value = position.get('add_position_total_value', 0)  # 补仓总金额
            
            if capital_after_entry is not None:
                # 🆕 修复补仓后余额计算：
                # 建仓后余额 - 补仓扣除的资金 + 返还的总本金 + 盈亏
                # 因为补仓时从capital_after_entry中扣除了资金，所以需要加回来
                capital_after_exit = capital_after_entry - add_position_total_value + position_value + pnl
            else:
                # 如果没有建仓后余额记录，使用全局余额（向后兼容）
                capital_after_exit = self.capital
            
            # 🔍 调试信息：验证平仓后余额的计算
            if capital_after_entry is not None:
                expected_after_exit = capital_after_entry - add_position_total_value + position_value + pnl
                logging.debug(f"🔍 平仓余额计算: {position['symbol']} 建仓后余额={capital_after_entry:.2f} 补仓总金额={add_position_total_value:.2f} 投入本金={position_value:.2f} 盈亏={pnl:.2f} 平仓后={capital_after_exit:.2f} 验证={expected_after_exit:.2f}")
            else:
                logging.debug(f"🔍 平仓余额计算: {position['symbol']} 建仓后余额=N/A 投入本金={position_value:.2f} 盈亏={pnl:.2f} 平仓后={capital_after_exit:.2f} (使用全局余额)")
            ...
            position.update({
                ...
                'pnl_pct': pnl_pct,
                'hold_days': hold_days,
                'capital_after_exit': capital_after_exit  # 🆕 记录平仓后的资金余额
            })
    ...
    def run_backtest(self, start_date: str, end_date: str):
        """🚀 向量化回测（一次性加载所有数据，使用pandas/numpy批量处理）
        
        优化点：
        1. 一次性加载所有历史数据到内存
        2. 向量化计算所有信号
        3. 按天推进回测，每天检查所有小时的数据
        4. 使用pandas批量操作替代循环
        """
        ...
        logging.info(f"🚀 开始向量化回测（按天推进）: {start_date} 到 {end_date}")
        ...
        # 步骤1：批量加载所有数据
        all_data = self.load_all_data_batch(start_date, end_date)
        
        if not all_data:
            logging.error("❌ 未加载到任何数据，回测终止")
            return
        
        # 步骤2：向量化计算所有信号
        all_signals_df = self.calculate_all_signals_vectorized(all_data, start_date, end_date)
        
        if all_signals_df.empty:
            logging.warning("⚠️ 未发现任何信号，回测终止")
            return
        ...
        while current_date <= end_dt:
            ...
            # 🆕 每天输出进度信息（每10天输出一次详细日志，减少日志量）
            if (current_date - datetime.strptime(start_date, '%Y-%m-%d')).days % 10 == 0 or current_date == end_dt:
                logging.info(f"📅 处理日期: {date_str} | 当前资金: ${self.capital:,.2f} | 持仓数: {len(self.positions)} | 待建仓信号: {len(self.pending_signals)}")
            else:
                logging.debug(f"📅 处理日期: {date_str} | 当前资金: ${self.capital:,.2f} | 持仓数: {len(self.positions)} | 待建仓信号: {len(self.pending_signals)}")
            ...
            # 将当前日期转换为当天的23:59:59，用于检查所有小时的数据
            current_datetime = current_date + timedelta(hours=23, minutes=59, seconds=59)
            ...
            for position in positions_to_check:
                try:
                    ...
                    if symbol in all_data:
                        # 使用已加载的数据检查持仓
                        hourly_df = all_data[symbol].copy()
                        hourly_df['trade_datetime'] = pd.to_datetime(hourly_df['trade_date'])
                        
                        # 筛选到当前日期23:59:59的数据
                        mask = hourly_df['trade_datetime'] <= current_datetime
                        current_data = hourly_df[mask].copy()
                        
                        if not current_data.empty:
                            # 检查是否满足平仓条件（遍历当天所有小时数据）
                            for _, row in current_data.iterrows():
                                hour_datetime = row['trade_datetime']
                                # 只检查当天的数据
                                if hour_datetime.date() == current_date.date():
                                    self._check_position_with_data(position, row, hour_datetime, all_data)
                except Exception as e:
                    logging.error(f"❌ 检查持仓失败 {position.get('symbol', 'unknown')}: {e}")
            ...
            for signal in self.pending_signals[:]:
                ...
                if current_date > signal['timeout_datetime']:
                    logging.info(f"⏰ [{date_str}] {symbol} 信号超时，放弃建仓（买量{signal['buy_surge_ratio']:.1f}倍）")
                ...
                if symbol in all_data:
                    ...
                    mask = (hourly_df['trade_datetime'] >= signal_datetime) & (hourly_df['trade_datetime'] <= current_datetime)
                    check_data = hourly_df[mask].copy()
                    
                    if not check_data.empty:
                        target_price = signal['signal_close'] * (1 + self.get_wait_drop_pct(signal['buy_surge_ratio']))
                        
                        # 检查是否有低点达到目标价格
                        if (check_data['low'] <= target_price).any():
                            # 找到第一个达到目标价格的小时
                            entry_row = check_data[check_data['low'] <= target_price].iloc[0]
                            entry_datetime = entry_row['trade_datetime']
                            ...
                            if len(self.positions) < self.max_daily_positions:
                                ...
                                self.execute_trade(symbol, entry_price, entry_date, 
                                                 signal['signal_date'], signal['buy_surge_ratio'], 
                                                 entry_datetime=entry_datetime)
                                if len(self.trade_records) > before_trades:
                                    logging.info(f"✅ {symbol} 达到目标跌幅，建仓价{entry_price:.6f}，建仓时间{entry_datetime}")
                                    self._update_signal_record(symbol, signal.get('signal_date'), 
                                                             status='entered', entry_datetime=entry_datetime,
                                                             entry_price=entry_price, note='触发目标价并建仓')
                                else:
                                    # execute_trade 内部可能因为资金/风控拒绝
                                    self._update_signal_record(symbol, signal.get('signal_date'),
                                                             status='reached_not_entered', entry_datetime=entry_datetime,
                                                             entry_price=entry_price, note='触发目标价但未建仓（资金/风控）')
            ...
            if len(self.positions) < self.max_daily_positions:
                # 筛选当天的信号
                day_signals = all_signals_df[
                    (all_signals_df['signal_datetime'].dt.date == current_date.date()) &
                    (~all_signals_df['symbol'].isin([s['symbol'] for s in self.pending_signals])) &
                    (~all_signals_df['symbol'].isin([p['symbol'] for p in self.positions]))
                ]
                
                for _, signal_row in day_signals.iterrows():
                    signal_datetime = signal_row['signal_datetime']
                    ...
                    # 🎯 根据买量倍数动态设置等待回调比例
                    target_drop_pct = self.get_wait_drop_pct(signal_row['surge_ratio'])
                    ...
                    self.pending_signals.append({
                        ...
                        'signal_close': signal_row['signal_price'],
                        'buy_surge_ratio': signal_row['surge_ratio'],
                    ...
                    try:
                        self.signal_records.append({
                            ...
                            'buy_surge_ratio': float(signal_row['surge_ratio']),
                            'target_drop_pct': float(target_drop_pct),
                            'target_price': float(signal_row['signal_price']) * (1 + float(target_drop_pct)),
                    ...
                    logging.info(f"🔔 新信号: {signal_row['symbol']} @{signal_datetime.strftime('%H:00')} 买量{signal_row['surge_ratio']:.2f}倍，可建仓时间: {earliest_entry_datetime.strftime('%H:00')}")
        ...
        for position in self.positions.copy():
            try:
                ...
                if symbol in all_data:
                    hourly_df = all_data[symbol].copy()
                    hourly_df['trade_datetime'] = pd.to_datetime(hourly_df['trade_date'])
                    end_dt_for_close = datetime.strptime(end_date, '%Y-%m-%d') + timedelta(hours=23, minutes=59, seconds=59)
                    mask = hourly_df['trade_datetime'] <= end_dt_for_close
                    if mask.any():
                        last_row = hourly_df[mask].iloc[-1]
                        exit_price = last_row['close']
        ...
        logging.info("✅ 回测完成")
        logging.info(f"⏰ 回测程序启动时间: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        logging.info(f"⏰ 回测程序结束时间: {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
        logging.info(f"⏱️  总耗时: {int(hours)}小时 {int(minutes)}分钟 {int(seconds)}秒 ({duration.total_seconds():.2f}秒)")

    def load_all_data_batch(self, start_date: str, end_date: str) -> Dict[str, pd.DataFrame]:
        """🚀 批量加载所有交易对的历史数据（一次性加载，避免重复查询）
        ...
        Returns:
            Dict[symbol, DataFrame]: 所有交易对的小时K线数据
        """
        logging.info(f"📦 开始批量加载数据: {start_date} 到 {end_date}")
        
        all_symbols = self.get_all_symbols()
        all_data = {}
        
        # 需要提前一天的数据（用于计算昨日平均小时买量）
        start_dt = datetime.strptime(start_date, '%Y-%m-%d')
        load_start_date = (start_dt - timedelta(days=1)).strftime('%Y-%m-%d')
        
        total_symbols = len(all_symbols)
        logging.info(f"📦 准备加载 {total_symbols} 个交易对的数据...")
        
        for idx, symbol in enumerate(all_symbols, 1):
            try:
                # 加载小时K线数据（包含前一天，用于计算昨日平均）
                hourly_df = self.get_hourly_kline_data(symbol, start_date=load_start_date, end_date=end_date)
                if not hourly_df.empty:
                    # 确保 trade_date 是 datetime 类型
                    if 'trade_date' in hourly_df.columns:
                        hourly_df['trade_datetime'] = pd.to_datetime(hourly_df['trade_date'])
                    all_data[symbol] = hourly_df
                
                # 每加载100个交易对显示一次进度
                if idx % 100 == 0:
                    logging.info(f"  数据加载进度: {idx}/{total_symbols} ({idx*100//total_symbols}%)")
            except Exception as e:
                logging.debug(f"加载 {symbol} 数据失败: {e}")
                continue
        
        logging.info(f"✅ 数据加载完成，共加载 {len(all_data)} 个交易对的数据")
        return all_data

    def calculate_all_signals_vectorized(self, all_data: Dict[str, pd.DataFrame], start_date: str, end_date: str) -> pd.DataFrame:
        """🚀 向量化计算所有信号（一次性计算，避免逐日循环）
        
        Args:
            all_data: 所有交易对的小时K线数据
            start_date: 开始日期
            end_date: 结束日期
        
        Returns:
            DataFrame: 所有信号，包含 symbol, signal_datetime, signal_price, surge_ratio 等
        """
        logging.info("🔍 开始向量化计算所有信号...")
        ...
        # 批量加载日K线数据（用于计算昨日平均小时买量）
        daily_data_cache = {}  # {symbol: {date: volume}}
        
        for symbol, hourly_df in all_data.items():
            if hourly_df.empty:
                continue
            ...
            try:
                ...
                with engine.connect() as conn:
                    result = conn.execute(text(f'''
                        SELECT trade_date, active_buy_volume
                        FROM {safe_table_name}
                ...
                for row in daily_rows:
                    date_str = row[0][:10] if len(row[0]) > 10 else row[0]  # 提取日期部分
                    daily_volumes[date_str] = row[1]
                daily_data_cache[symbol] = daily_volumes
            except:
                continue
        
        # 向量化计算每个交易对的信号
        for symbol, hourly_df in all_data.items():
            if symbol not in daily_data_cache:
                continue
            
            daily_volumes = daily_data_cache[symbol]
            if not daily_volumes:
                continue
            
            # 筛选回测日期范围内的数据
            hourly_df['trade_datetime'] = pd.to_datetime(hourly_df['trade_date'])
            mask = (hourly_df['trade_datetime'] >= start_dt) & (hourly_df['trade_datetime'] <= end_dt)
            period_df = hourly_df[mask].copy()
            
            if period_df.empty:
                continue
            
            # 为每行数据计算昨日平均小时买量
            period_df['date'] = period_df['trade_datetime'].dt.date.astype(str)
            period_df['yesterday_date'] = (period_df['trade_datetime'] - pd.Timedelta(days=1)).dt.date.astype(str)
            
            # 计算昨日平均小时买量
            def get_yesterday_avg_volume(row):
                yesterday_date = row['yesterday_date']
                if yesterday_date in daily_volumes and daily_volumes[yesterday_date]:
                    return daily_volumes[yesterday_date] / 24.0
                return None
            
            period_df['yesterday_avg_hour_volume'] = period_df.apply(get_yesterday_avg_volume, axis=1)
            
            # 过滤掉没有昨日数据的行
            period_df = period_df[period_df['yesterday_avg_hour_volume'].notna()]
            
            if period_df.empty:
                continue
            
            # 计算买量暴涨倍数
            period_df['surge_ratio'] = period_df['active_buy_volume'] / period_df['yesterday_avg_hour_volume']
            
            # 筛选满足条件的信号（2倍到max倍之间）
            signal_mask = (period_df['surge_ratio'] >= threshold) & (period_df['surge_ratio'] <= self.buy_surge_max)
            signals_df = period_df[signal_mask].copy()
            
            if signals_df.empty:
                continue
            
            # 每个交易对每天只保留第一个信号（按时间排序）
            signals_df = signals_df.sort_values('trade_datetime')
            signals_df = signals_df.groupby('date').first().reset_index()
            
            # 添加到信号列表
            for _, row in signals_df.iterrows():
                all_signals.append({
                    'symbol': symbol,
                    'signal_datetime': row['trade_datetime'],
                    'signal_price': row['close'],
                    'surge_ratio': row['surge_ratio'],
                    'signal_hour_volume': row['active_buy_volume'],
                    'yesterday_avg_hour_volume': row['yesterday_avg_hour_volume']
                })
        
        signals_df = pd.DataFrame(all_signals)
        if not signals_df.empty:
            signals_df = signals_df.sort_values(['signal_datetime', 'surge_ratio'], ascending=[True, False])
            logging.info(f"✅ 信号计算完成，共发现 {len(signals_df)} 个信号")
        else:
            logging.info("✅ 信号计算完成，未发现信号")
        
        return signals_df

    def _check_position_with_data(self, position: Dict, current_row: pd.Series, current_hour: datetime, all_data: Dict[str, pd.DataFrame]):
        """使用已加载的数据检查持仓是否满足平仓条件（与hm1.py逻辑一致）"""
        try:
            # 🔧 关键修复：如果持仓已经平仓，直接返回，避免重复检查
            if position.get('exit_date'):
                return
            ...
            if position.get('entry_datetime'):
                ...
                entry_datetime = datetime.strptime(position['entry_date'], '%Y-%m-%d')
            
            # 🔧 关键修复：确保只使用建仓时间之后的数据
            # 将 entry_datetime 转换为 pandas Timestamp 以确保类型一致
            if isinstance(entry_datetime, datetime):
                entry_datetime = pd.to_datetime(entry_datetime)
            elif isinstance(entry_datetime, str):
                entry_datetime = pd.to_datetime(entry_datetime)
            
            # 确保 current_hour 也是 pandas Timestamp
            if isinstance(current_hour, datetime):
                current_hour_ts = pd.to_datetime(current_hour)
            else:
                current_hour_ts = pd.to_datetime(current_hour)
            
            # 筛选建仓后到当前小时的数据（严格使用 >= 和 <= 确保只包含建仓时间之后的数据）
            mask = (hourly_df['trade_datetime'] >= entry_datetime) & (hourly_df['trade_datetime'] <= current_hour_ts)
            hold_period_data = hourly_df[mask].copy()
            
            if hold_period_data.empty:
                return
            
            # 🔧 关键修复：确保数据按时间排序，并且只包含建仓时间之后的数据
            hold_period_data = hold_period_data.sort_values('trade_datetime')
            
            # 🔧 再次验证：确保第一条数据的时间 >= 建仓时间
            if not hold_period_data.empty:
                first_row_time = hold_period_data.iloc[0]['trade_datetime']
                if first_row_time < entry_datetime:
                    # 如果第一条数据的时间早于建仓时间，说明数据筛选有问题，跳过
                    logging.warning(f"⚠️ 数据筛选错误: {symbol} 第一条数据时间 {first_row_time} 早于建仓时间 {entry_datetime}")
                    return
            
            # 🔧 关键修复：遍历所有历史小时数据，按时间顺序检查（与hm1.py逻辑一致）
            # 性能优化：限制检查的小时数，避免数据量过大时卡死
            # 注意：使用从头开始的数据，而不是 tail（tail 会跳过早期数据）
            max_hours_to_check = 1000  # 最多检查1000小时（约42天）
            if len(hold_period_data) > max_hours_to_check:
                # 如果数据超过1000小时，只检查最近1000小时（从后往前）
                rows_to_check = hold_period_data.tail(max_hours_to_check)
            else:
                # 否则检查所有数据
                rows_to_check = hold_period_data
            
            # 获取建仓时的具体小时时间戳（用于精确计算持仓小时数）
            # 🔧 使用 entry_datetime 而不是数据的第一条，确保时间准确
            ...
            for idx, row in rows_to_check.iterrows():
                ...
                # 🔧 关键修复：确保只检查建仓时间之后的数据
                # 将 hour_datetime 转换为 pandas Timestamp 以确保类型一致
                if isinstance(hour_datetime, (datetime, pd.Timestamp)):
                    hour_datetime_ts = pd.to_datetime(hour_datetime)
                else:
                    hour_datetime_ts = pd.to_datetime(hour_datetime)
                
                # 🔧 严格验证：跳过建仓时间之前的数据
                if hour_datetime_ts < entry_datetime:
                    continue  # 跳过建仓时间之前的数据
                
                high_price = row['high']
                low_price = row['low']
                hour_date = hour_datetime_ts.strftime('%Y-%m-%d')
                hour_datetime_str = hour_datetime_ts.strftime('%Y-%m-%d %H:%M:%S')
                
                # 🔧 精确计算持仓小时数（基于小时时间戳）
                if entry_hour_timestamp is not None:
                    # 确保 entry_hour_timestamp 也是 pandas Timestamp
                    if isinstance(entry_hour_timestamp, (datetime, pd.Timestamp)):
                        entry_ts = pd.to_datetime(entry_hour_timestamp)
                    else:
                        entry_ts = pd.to_datetime(entry_hour_timestamp)
                    
                    hours_held_precise = (hour_datetime_ts - entry_ts).total_seconds() / 3600
                    
                    # 🔧 验证持仓小时数必须 >= 0
                    if hours_held_precise < 0:
                        logging.warning(f"⚠️ 持仓小时数为负数: {symbol} 建仓时间 {entry_ts} 当前时间 {hour_datetime_ts} 差值 {hours_held_precise:.2f}小时")
                        continue  # 跳过异常数据
                ...
                # ==========================
                # 🆕 动态止盈阈值（避免"偷看未来"：只有窗口走完才允许触发动态加成）
                # 🔧 确保传入的时间戳是 pandas Timestamp 类型
                dynamic_tp_pct = self.calculate_dynamic_take_profit(position, hourly_df, entry_datetime, hour_datetime_ts)
                tp_price = current_avg_price * (1 + dynamic_tp_pct)
                sl_price = current_avg_price * (1 + self.stop_loss_pct)
                add_price = current_avg_price * (1 + self.add_position_trigger_pct)
            ...
            hours_held = (current_hour_ts - entry_datetime).total_seconds() / 3600
            
            # 🔧 验证持仓小时数必须 >= 0
            if hours_held < 0:
                logging.warning(f"⚠️ 备用检查：持仓小时数为负数: {symbol} 建仓时间 {entry_datetime} 当前时间 {current_hour_ts} 差值 {hours_held:.2f}小时")
                return  # 跳过异常数据
            ...
            if hours_held >= self.max_hold_hours:
                if not hold_period_data.empty:
                    ...
                    exit_price = current_row.get('close', 0)
                
                hour_datetime_str = current_hour_ts.strftime('%Y-%m-%d %H:%M:%S')
                self.exit_position(position, exit_price, hour_datetime_str, "max_hold_time")
                logging.info(f"⏰ {self.max_hold_hours:.0f}小时强制平仓: {symbol} 持有{hours_held:.1f}小时，平仓价{exit_price:.6f}")
                return
        ...
        try:
            with open(csv_filename, 'w', newline='', encoding='utf-8-sig') as csvfile:
                ...
                if not entered_trades:
                    print(f"⚠️ 警告：没有找到已建仓的交易记录，CSV文件将只包含表头")
                    print(f"   总交易记录数: {len(self.trade_records)}")
                ...
                for i, trade in enumerate(entered_trades, 1):
                    ...
                    row = {
                        ...
                        # 🆕 仓位余额：建仓后和平仓后的剩余资金
                        '建仓后余额': f"{trade.get('capital_after_entry', 0):.2f}" if trade.get('capital_after_entry') is not None else '',
                        '平仓后余额': f"{trade.get('capital_after_exit', 0):.2f}" if trade.get('capital_after_exit') is not None else '',
            ...
            print(f"   共写入 {len(entered_trades)} 条交易记录")
    ...
    def generate_signal_csv_report(self):
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        csv_filename = csv_dir / f"buy_surge_signal_feedback_{timestamp}.csv"
...
def main():
    ...
    parser.add_argument(
        '--add-position-trigger-pct',
        type=float,
        default=None,
        help='补仓触发比例（默认-0.18，即-18%）。当价格从建仓价下跌到此比例时触发补仓'
    )

    parser.add_argument(
        '--add-position-size-ratio',
        type=float,
        default=None,
        help='补仓占资金比例（默认0.05，即5%）。补仓时使用的资金占当前资金的比例'
    )
    ...
    # 控制补仓功能
    if args.disable_add_position:
        backtest.enable_add_position = False
        logging.info("补仓功能已禁用")
    else:
        # 默认启用补仓（已在 __init__ 中设置）
        logging.info("补仓功能已启用（默认）")

    # 补仓触发比例
    if args.add_position_trigger_pct is not None:
        backtest.add_position_trigger_pct = float(args.add_position_trigger_pct)
        logging.info(f"补仓触发比例设置为: {backtest.add_position_trigger_pct*100:.1f}%")

    # 补仓占资金比例
    if args.add_position_size_ratio is not None:
        backtest.add_position_size_ratio = float(args.add_position_size_ratio)
        logging.info(f"补仓占资金比例设置为: {backtest.add_position_size_ratio*100:.1f}%")
