...
class StandardBacktest:
    ...
    def __init__(self):
        ...
        self.leverage = 5.0  # 杠杆倍数
        self.profit_threshold = 0.20  # 止盈比例 (20%)
        self.loss_threshold = 0.19  # 止损比例 (19%)
        self.position_size_ratio = 0.05  # 单次建仓占资金比例
        self.min_pct_chg = 0.1  # 最小涨幅百分比（小数，如0.1表示10%）
        self.delay_entry = False  # 是否启用延迟入场策略
        self.delay_hours = 12  # 延迟入场的小时数
...
"""
    ...
    def get_top_gainer_by_date(self, date: str) -> Optional[Tuple[str, float]]:
        """
        获取指定日期涨幅第一的交易对
    ...
    def get_all_top_gainers(self, start_date: str, end_date: str) -> pd.DataFrame:
        """
    ...
    def get_kline_data_for_date(self, symbol: str, date: str) -> Optional[pd.Series]:
        """
        获取指定交易对在指定日期的K线数据
    ...
    def get_hourly_kline_data_for_date(self, symbol: str, date: str, hours: int = 12) -> Optional[pd.DataFrame]:
        """
    ...
    def get_daily_hourly_kline_data(self, symbol: str, date: str) -> Optional[pd.DataFrame]:
        """
    ...
    def check_momentum_weakening(self, hourly_df: pd.DataFrame, min_hours: int = 3) -> Tuple[bool, Optional[float]]:
        """
        检查涨势是否减弱
    ...
    def run_backtest(
        ...
        _profit_threshold = self.profit_threshold
        _loss_threshold = self.loss_threshold
        _position_size_ratio = self.position_size_ratio
        _min_pct_chg = self.min_pct_chg
        ...
        top_gainers_df = self.get_all_top_gainers(start_date, end_date)
        ...
        if self.delay_entry:
            logging.info(f"延迟入场策略: 启用，延迟{self.delay_hours}小时，等待涨势减弱后建仓")
        ...
        while current_date <= end_dt:
            ...
            for i, current_position in enumerate(current_positions):
                ...
                # 获取当天的小时线数据（用于止盈止损判断）
                hourly_df = self.get_daily_hourly_kline_data(symbol, date_str)
                ...
                if hourly_df is None or hourly_df.empty:
                    ...
                    kline_data = self.get_kline_data_for_date(symbol, date_str)
                    ...
                    if price_change_low <= -_profit_threshold:
                        ...
                        if add_position_count < MAX_ADD_POSITION_COUNT:
                            ...
                            add_position_value = self.capital * _position_size_ratio
                            
                            # 检查资金是否足够
                            if self.capital < add_position_value:
                                logging.warning(f"{date_str}: {symbol} 资金不足，无法补仓。当前资金: {self.capital:.2f} USDT，需要: {add_position_value:.2f} USDT")
                                # 资金不足，直接止损
                    ...
                    if exit_price:
                        ...
                        entry_reason = current_position.get('entry_reason', 'immediate' if not self.delay_entry else 'unknown')
                        delay_hours_used = self.delay_hours if is_delay_entry else None
            ...
            # 处理延迟入场的待建仓列表
            if self.delay_entry and pending_positions:
                pending_to_remove = []
                for i, pending in enumerate(pending_positions):
                    ...
                    if date_str == target_date:
                        # 获取1小时K线数据
                        hourly_df = self.get_hourly_kline_data_for_date(symbol, date_str, hours=self.delay_hours)
                        
                        if hourly_df is not None and len(hourly_df) >= 3:
                            # 检查涨势是否减弱
                            can_entry, entry_price = self.check_momentum_weakening(hourly_df, min_hours=3)
                            
                            if can_entry and entry_price:
                                # 涨势减弱，可以建仓
                                position_size = (self.capital * _position_size_ratio * _leverage) / entry_price
                                position_value = self.capital * _position_size_ratio
                                ...
                                if hours_passed >= self.delay_hours:
                                    # 超时，强制建仓（使用当前价格）
                                    kline_data = self.get_kline_data_for_date(symbol, date_str)
                                    if kline_data is not None:
                                        ...
                                        if self.capital >= position_value:
                                            ...
                                            logging.info(
                                                ...
                                                f"建仓价: {entry_price:.8f} | "
                                                f"原因: 超过延迟时间{self.delay_hours}小时 | "
                                                f"持仓数量: {position_size:.4f} | "
                                                f"建仓金额: {position_value:.2f} USDT"
                                            )
                ...
                if pct_chg >= _min_pct_chg * 100 and not already_holding and not already_pending:
                    if self.delay_entry:
                        ...
                        if next_date <= end_dt:
                            ...
                            logging.info(
                                f"{date_str}: 发现涨幅第一 {symbol} (涨幅{pct_chg:.2f}%)，加入待建仓列表，将在{next_date_str}开始监控（延迟{self.delay_hours}小时）"
                            )
                            delay_entry_stats['total_pending'] += 1
                    else:
                        # 立即建仓策略（原有逻辑）
                        ...
                        if next_date <= end_dt:
                            kline_data = self.get_kline_data_for_date(symbol, next_date_str)
                            if kline_data is not None:
                                # 建仓价使用开盘价
                                ...
                                position_value = self.capital * _position_size_ratio  # 建仓金额
        ...
        if current_positions:
            ...
            for current_position in current_positions:
                ...
                kline_data = self.get_kline_data_for_date(symbol, last_date_str)
                if kline_data is not None:
        ...
        if self.trade_records:
            ...
            csv_filename = csv_dir / f"backtrade_records_{start_date}_{end_date}.csv"
            ...
            total_profit_loss = self.capital - _initial_capital  # 总盈亏 = 最终资金 - 初始资金
            ...
            logging.info(f"总收益率: {(self.capital - _initial_capital) / _initial_capital * 100:.2f}%")
            ...
            result = {
                ...
                'total_return_rate': (self.capital - _initial_capital) / _initial_capital * 100,
