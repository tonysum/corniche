...
【7️⃣ CSV报告生成（generate_trade_csv_report函数）】

  📊 报告内容：
    • 交易明细：序号、交易对、买量暴涨倍数、信号时间、建仓/平仓信息
    • 盈亏信息：盈亏金额、盈亏百分比、平仓原因
    • 补仓信息：是否有补仓、补仓价格、补仓后平均价
    • 持仓统计：持仓小时数、最大跌幅%、2小时/24小时最大涨幅%、止盈阈值%
    • 顶级交易者数据：建仓时/平仓时账户多空比、账户多空比变化
    • 未平仓持仓：当前5m时间、收盘价、浮盈金额、浮盈百分比

  💰 盈亏金额计算逻辑：
    ① 正常交易（非虚拟补仓）：
       • 计算公式：盈亏金额 = (平仓价 - 建仓价) / 建仓价 × 仓位金额 × 杠杆倍数
       • 示例：建仓价100，平仓价110，仓位100，杠杆4倍
         → 盈亏 = (110-100)/100 × 100 × 4 = 40
    
    ② 虚拟补仓交易（is_virtual_tracking=True）：
       • 计算公式：盈亏金额 = 首次投入金额 × (-0.72) = -72%本金
       • 说明：虚拟补仓时首仓在-18%触发，记录首仓实际亏损
       • 示例：首次投入100，虚拟补仓后盈亏 = 100 × (-0.72) = -72
    
    ③ 观察模式平仓：
       • 计算公式：与正常交易相同
       • 说明：资金已在进入观察模式时返还，平仓时只计算盈亏

  📝 补仓信息记录：
    • 是否有补仓：✅是 / 否
    • 补仓价格：虚拟补仓使用触发补仓时的价格（current_price）
    • 补仓后平均价：
      - 虚拟补仓：使用virtual_entry_price（虚拟平均建仓价）
      - 实际补仓：使用avg_entry_price（补仓后更新的平均成本）

  🔍 平仓价格记录：
    • 正常交易：使用exit_price（实际平仓价格）
    • 虚拟补仓交易：使用exit_price（实际平仓价格，而非real_exit_price）
    • 注意：real_exit_price仅用于计算首仓亏损，不作为平仓价显示
...
创建时间：2026-01-11
最后更新：2026-01-23
版本：v3.1（修复虚拟补仓平仓价格计算，完善CSV报告）

主要更新：
  • v3.1 (2026-01-23): 
    - 修复虚拟补仓交易平仓价格计算逻辑（使用实际平仓价而非补仓触发价）
    - 修复虚拟补仓后同小时止盈检查问题（不允许同小时止盈）
    - 完善CSV报告：补仓价格、补仓后平均价正确填写
    - 优化虚拟补仓后止盈/止损价计算（基于虚拟建仓价）
...
class BuySurgeBacktest:
    ...
    def check_signal_surge(self, symbol: str, signal_date: str, signal_close: float) -> tuple:
        ...
        try:
            ...
            with engine.connect() as conn:
                result = conn.execute(query, {"signal_date": signal_date_str})
                row = result.fetchone()
        ...
        try:
            ...
            if current_datetime >= window_2h_end:
                ...
                try:
                    ...
                    safe_table_name = f'"{kline_5m_table}"'
                    
                    # 获取建仓后2小时内的5分钟K线（24根）
                    start_dt = entry_datetime
                    end_dt = window_2h_end
            ...
            if current_datetime >= window_12h_end:
                ...
                try:
                    ...
                    safe_table_name = f'"{hourly_table}"'
                    
                    # 获取12小时后附近的K线（允许前后1小时的误差）
                    window_start_dt = window_12h_end
                    window_end_dt = window_12h_end + timedelta(hours=1)
    ...
    def get_all_symbols(self) -> List[str]:
        ...
        # PostgreSQL 使用 information_schema 查询表
        query = text("""
            SELECT table_name 
        ...
        # 去掉前缀 'K1d'，例如 'K1dBTCUSDT' -> 'BTCUSDT'
        symbols = [
            table_name[0][3:]  # 去掉 'K1d' 前缀
            for table_name in tables 
    ...
    def get_hourly_kline_data(self, symbol: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            if start_date and end_date:
                query = text(f'SELECT * FROM {safe_table_name} WHERE trade_date >= :start_date AND trade_date <= :end_date ORDER BY trade_date ASC')
                params = {"start_date": start_date, "end_date": end_date + ' 23:59:59'}
            elif start_date:
                query = text(f'SELECT * FROM {safe_table_name} WHERE trade_date >= :start_date ORDER BY trade_date ASC')
                params = {"start_date": start_date}
            elif end_date:
                query = text(f'SELECT * FROM {safe_table_name} WHERE trade_date <= :end_date ORDER BY trade_date ASC')
                params = {"end_date": end_date + ' 23:59:59'}
            else:
                # 没有指定范围时，查询全部（但会很慢）
                logging.warning(f"查询 {symbol} 全部小时K线数据，可能较慢")
                query = text(f'SELECT * FROM {safe_table_name} ORDER BY trade_date ASC')
                params = {}
                ...
                for idx, row in hold_period_data.iterrows():
                    ...
                    if self.enable_add_position and not position.get('has_add_position', False):
                        ...
                        if low_price <= add_price:
                            ...
                            # 🆕 虚拟补仓后，需要重新计算current_avg_price和止盈/止损价
                            if self.use_virtual_add_position:
                                # 虚拟补仓后，更新current_avg_price为虚拟建仓价
                                if position.get('is_virtual_tracking', False) and position.get('virtual_entry_price'):
                                    current_avg_price = position['virtual_entry_price']
                                    # 重新计算止盈/止损价（基于虚拟建仓价）
                                    dynamic_tp_pct = self.calculate_dynamic_take_profit(position, hourly_df, entry_datetime, hour_datetime)
                                    tp_price = current_avg_price * (1 + dynamic_tp_pct)
                                    sl_price = current_avg_price * (1 + self.stop_loss_pct)
                                    # 🔧 虚拟补仓后，不允许同小时止盈，只检查止损
                                    if low_price <= sl_price:
                                        self.exit_position(position, sl_price, hour_datetime_str, "stop_loss")
                                        logging.warning(
                                            f"🛑 虚拟补仓后同小时止损: {symbol} low={low_price:.6f} 触发止损阈值，按止损价{sl_price:.6f}成交"
                                        )
                                        return True
                                # 虚拟补仓后，跳过当前K线的止盈检查，从下一小时开始
                                continue
                            else:
                                # 实际补仓后重新计算止损价（使用更新后的平均成本）
                                current_avg_price = position['avg_entry_price']
    ...
    def add_position(self, position: Dict, current_price: float, current_date: str):
        ...
        try:
            if self.use_virtual_add_position:
                ...
                # 🆕 设置补仓价格（用于CSV报告）
                # 虚拟补仓时，补仓价格使用触发补仓时的价格（current_price）
                # 但为了与回测逻辑一致，也可以使用阈值价格，这里使用实际触发价格
                position['add_position_price'] = current_price  # 🆕 记录补仓触发价格
    ...
    def get_latest_5m_close(self, symbol: str, asof_dt: Optional[datetime] = None):
        ...
        try:
            ...
            # PostgreSQL 使用 information_schema 检查表是否存在
            check_query = text("""
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_schema = 'public' 
            ...
            with engine.connect() as conn:
                result = conn.execute(query, {"asof_str": asof_str})
                row = result.fetchone()
    ...
    def get_5m_closes_in_window(self, symbol: str, start_dt: datetime, end_dt: datetime) -> List[float]:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                closes = [row[0] for row in result.fetchall() if row[0] is not None]
            
            return closes
        except Exception as e:
            logging.debug(f"获取5m K线数据失败 {symbol}: {e}")
            return []
    ...
    def run_backtest(self, start_date: str, end_date: str):
        ...
        for position in self.positions.copy():
            try:
                cursor = self.crypto_conn.cursor()
                table_name = f'DailyKline_{position["symbol"]}'
    ...
    def generate_trade_csv_report(self):
        ...
        # __file__ 是 backend/hm20260122.py，所以需要向上两级到项目根目录
        ...
        try:
            with open(csv_filename, 'w', newline='', encoding='utf-8-sig') as csvfile:
                ...
                for i, trade in enumerate(entered_trades, 1):
                    # 🆕 计算补仓后平均价
                    avg_price_after_add = ''
                    if trade.get('has_add_position', False):
                        # 虚拟补仓：使用virtual_entry_price（虚拟平均建仓价）
                        if trade.get('is_virtual_tracking', False) and trade.get('virtual_entry_price'):
                            avg_price_after_add = f"{trade['virtual_entry_price']:.6f}"
                        # 实际补仓：使用avg_entry_price（补仓后更新的平均成本）
                        elif trade.get('add_position_price'):
                    ...
                    row = {
                        ...
                        # 🆕 平仓价：使用实际的平仓价格（exit_price）
                        # 注意：real_exit_price只是虚拟补仓触发时的价格（用于计算首仓亏损），不应该作为平仓价
                        '平仓价': (
                        ...
                        # 🆕 补仓价格：虚拟补仓和实际补仓都使用add_position_price
                        '补仓价格': (
                            f"{trade.get('add_position_price', 0):.6f}" 
                            if trade.get('has_add_position', False) and trade.get('add_position_price')
                            else ''
                        ),
                        # 🆕 补仓后平均价：虚拟补仓用virtual_entry_price，实际补仓用avg_entry_price
