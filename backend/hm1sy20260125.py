...
# 数据库路径
# CRYPTO_DB_PATH = "db/crypto_data.db"
...
class BuySurgeBacktest:
    ...
    def __init__(self):
        ...
        self.position_size_ratio = 0.01  # 单次建仓占资金比例（6%）
        self.add_position_size_ratio = 0.05  # 补仓占资金比例（5%，可以设置为首仓的倍数）
        self.max_daily_positions = 20  # 并发持仓上限（保守设置，留出缓冲空间）
        ...
        logging.info(f"   max_daily_positions = {self.max_daily_positions} (应该是20)")
    ...
    def check_signal_surge(self, symbol: str, signal_date: str, signal_close: float) -> tuple:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                result = conn.execute(query, {"signal_ts": signal_ts})
                row = result.fetchone()
            ...
            if current_datetime >= window_12h_end:
                ...
                try:
                    ...
                    with self.engine.connect() as conn:
                        ...
                        LIMIT 1
                        """)
                        result = conn.execute(query, {"window_start_ts": window_start_ts, "window_end_ts": window_end_ts})
                        row = result.fetchone()
        ...
        try:
            with self.engine.connect() as conn:
                ...
                for table_name_row in tables:
                    ...
                    symbol = table_name.replace('K1d', '').upper()
                    ...
                    try:
                        ...
                        today_result = conn.execute(query_data, {
                            "date_str": date_str,
                            "date_pattern": f'{date_str}%'
                        })
                        today_row = today_result.fetchone()
                        
                        if not today_row or not today_row[3]:
                            continue
                        
                        today_date, close_price, open_price, today_buy_volume = today_row
                        ...
                        yesterday_buy_volume = yesterday_row[0]
                        ...
                        logging.debug(f"处理 {symbol} 信号失败: {e}")
                        continue
    ...
    def get_all_symbols(self) -> List[str]:
        ...
        symbols = [
            table_name[0].replace('K1d', '').upper() 
            for table_name in tables 
            if table_name[0].replace('K1d', '').upper().endswith('USDT')
        ]
        self._all_symbols_cache = sorted(list(set(symbols)))
        logging.info(f"🔍 找到 {len(self._all_symbols_cache)} 个USDT交易对")
        return self._all_symbols_cache
            ...
            with self.engine.connect() as conn:
                for idx, symbol in enumerate(all_symbols, 1):
                    try:
                        ...
                        yesterday_result = conn.execute(query_daily, {
                        ...
                        today_result = conn.execute(query_hourly, {
                            "check_pattern": f'{check_date}%'
                        })
                        today_hours = today_result.fetchall()
                        if not today_hours:
                            continue
    ...
    def get_hourly_kline_data(self, symbol: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            if start_date and end_date:
                query = f'SELECT * FROM "{table_name}" WHERE trade_date >= :start_date AND trade_date <= :end_date ORDER BY trade_date ASC'
                params = {"start_date": start_date, "end_date": end_date + ' 23:59:59'}
            elif start_date:
                query = f'SELECT * FROM "{table_name}" WHERE trade_date >= :start_date ORDER BY trade_date ASC'
                params = {"start_date": start_date}
            elif end_date:
                query = f'SELECT * FROM "{table_name}" WHERE trade_date <= :end_date ORDER BY trade_date ASC'
    ...
    def get_latest_5m_close(self, symbol: str, asof_dt: Optional[datetime] = None):
        ...
        数据来源：PostgreSQL 的 `K5m{symbol}` 表。
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                """)
                result_exists = conn.execute(query_exists, {"table_name": table_name})
                if not result_exists.fetchone()[0]:
                    return None, None
                ...
                query_close = text(f'''
                    ...
                    LIMIT 1
                ''')
                result_close = conn.execute(query_close, {"asof_str": asof_str})
                row = result_close.fetchone()
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                query_closes = text(f'''
                    ...
                    ORDER BY trade_date ASC
                ''')
                result_closes = conn.execute(query_closes, {"start_str": start_str, "end_str": end_str})
                rows = result_closes.fetchall()
