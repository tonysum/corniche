...
【1️⃣ 信号发现】
  ...
  🆕 顶级交易者数据筛选（默认启用）：
    - 筛选条件：账户多空比 >= 0.70（可通过 --min-account-ratio 配置）
    - 数据来源：Binance顶级交易者持仓数据（每日00:00采集）
    - 筛选逻辑：
      ✓ 有数据且账户多空比 >= 0.70 → 放行
      ✓ 无数据 → 放行（不因数据缺失错失机会）
      ✗ 有数据但账户多空比 < 0.70 → 过滤掉
    - 查询窗口：±24小时（匹配每日数据采集频率）
    - 过滤效果：约过滤15-20%的信号，有效减少低质量交易
...
【4️⃣ 补仓机制（支持虚拟补仓模式）】
  ...
  🆕 虚拟补仓模式（默认启用）：
    - 核心理念：不实际追加资金，但保留补仓的战略价值
    - 运作机制：
      ✓ 触发补仓时不追加资金（避免补仓亏损）
      ✓ 计算虚拟的新平均成本（仿佛补仓了）
      ✓ 后续止损/止盈基准使用虚拟平均成本
      ✓ 仍然占用一个仓位槽（保留"资金节流阀"效应）
      ✓ 实际盈亏基于原始成本和仓位（不增加实际风险）
    - 战略价值：
      ① 避免实际追加资金，减少单笔亏损风险
      ② 调整止损/止盈基准，给失败交易一个"虚拟救活"的机会
      ③ 占用仓位槽，防止冒进建立低质量新仓（节流阀效应）
      ④ 实测数据：虚拟补仓版本收益比无补仓版本高64%
    
  传统补仓模式（可选）：
    - 实际追加资金：当前资金 × 5%
    - 成本重算：平均成本 = (首次建仓价 × 首次数量 + 补仓价 × 补仓数量) / 总数量
...
【5️⃣ 止损机制】
  ...
  🆕 ② 顶级交易者动态止损（实时监控）：
    - 启用条件：默认启用（可通过命令行参数控制）
    - 监控指标：账户多空比（Long/Short Ratio）变化
    - 止损触发：账户多空比从建仓时下降 >= 0.10（可通过 --account-stop-threshold 配置）
    - 止损方式：使用当前小时收盘价平仓
    - 优先级：在价格止损之前检查（更早介入）
    - 设计理念：
      * 当顶级交易者从看多转向看空时提前止损
      * 平均止损损失仅 -$11/笔，远低于价格止损的 -$868/笔
      * 有效避免深度回撤，保护本金
    - 典型案例：JELLYJELLYUSDT 账户多空比从1.9780降到1.7663，在-8.1%时止损

【6️⃣ 24小时弱势平仓】
  ⚠️ 已禁用（数据证明效果不佳）
  • 问题分析：
    - 24h涨幅<8%的币，继续持有往往触发更大的止损
    - 24h涨幅3-8%的币看似有涨幅，实际是"温水煮青蛙"
    - 提前平仓反而效果更好（-$27/笔 vs 继续持有后-$84/笔）
  • 结论：依赖动态止盈、止损、顶级交易者止损即可

【7️⃣ 强制平仓】
  • 最大持仓时间：48小时（2天）
  • 平仓方式：超过48小时后，使用当前小时收盘价强制平仓
  • 设计理念：避免长期占用资金，提升资金周转率

═══════════════════════════════════════════════════════════════════════════════
📈 回测表现对比（2025-12-11 至 2026-01-16）
═══════════════════════════════════════════════════════════════════════════════
...
总收益率：+47.81%
最大回撤：38.58%
最终资金：$14,781.48（初始$10,000）

平仓原因分布：
  • 动态止盈(>10%)：72次（62.6%）
  • 价格止损：11次（9.6%，损失-$10,534）
  • 超时平仓：32次（27.8%）
...
总收益率：+45.62%
最大回撤：30.08% ⬇️ 降低22%
最终资金：$14,561.58（初始$10,000）

平仓原因分布：
  • 动态止盈(>10%)：40次（41.7%）
  • 🆕 顶级交易者止损：40次（平均损失仅 -$11.46/笔）✨
  • 超时平仓：14次（14.6%）
  • 价格止损：4次（4.2%，损失-$3,609）

风控效果对比：
  ✅ 收益率仅损失 2.19%（从47.81%降到45.62%）
  ✅ 最大回撤降低 22%（从38.58%降到30.08%）
  ✅ 风险收益比提升 22.6%（从1.24提升到1.52）
  ✅ 止损损失减少 66%（从-$10,533降到-$3,609）
  ✅ 顶级交易者止损触发40次，有效控制单笔亏损
  ✅ 价格止损次数减少63.6%（从11次降到4次）
...
class BuySurgeBacktest:
    ...
    def __init__(self):
        ...
        # 🆕 添加：连接顶级交易者数据库（PostgreSQL，用于读取历史数据）
        # 使用 PostgreSQL engine（连接池管理，无需手动关闭）
        logging.debug(f"🔍 [DEBUG] 初始化 trader_engine，engine={engine}")
        self.trader_engine = engine
        logging.debug(f"🔍 [DEBUG] trader_engine 已设置: {self.trader_engine}")
        try:
            # 测试连接并检查表是否存在
            with self.trader_engine.connect() as conn:
                result = conn.execute(
                    text("""
                        SELECT EXISTS (
                            SELECT FROM information_schema.tables 
                            WHERE table_schema = 'public' 
                            AND table_name = 'top_account_ratio'
                        );
                    """)
                )
                table_exists = result.fetchone()[0]
                if table_exists:
                    logging.info(f"✅ 已连接顶级交易者数据库（PostgreSQL），表 top_account_ratio 存在")
                else:
                    logging.warning(f"⚠️  顶级交易者数据表 top_account_ratio 不存在，将跳过多空比风控")
                    self.trader_engine = None
        except Exception as e:
            logging.warning(f"⚠️  无法连接顶级交易者数据库: {e}，将跳过多空比风控")
            self.trader_engine = None

        # 🆕 添加：顶级交易者风控参数
        self.enable_trader_filter = True  # 是否启用顶级交易者过滤（默认开启）
        self.min_account_ratio = 0.70  # 最小账户多空比（信号筛选）
        self.account_ratio_stop_threshold = -0.10  # 账户多空比下降止损阈值
        ...
        # 🆕 虚拟补仓补偿机制
        self.virtual_add_compensation_multiplier = 2.0  # 虚拟补仓后，下次建仓投入金额的补偿系数
        ...
        self.max_daily_positions = 10  # 并发持仓上限
    ...
    def __del__(self):
        """析构函数（PostgreSQL 使用连接池，不需要手动关闭连接）"""
        pass
    ...
    def get_top_trader_account_ratio(self, symbol: str, timestamp: datetime) -> Optional[Dict]:
        ...
        # 🔍 调试断点：方法入口
        import traceback
        logging.debug(f"🔍 [DEBUG] get_top_trader_account_ratio 调用: symbol={symbol}, timestamp={timestamp}, type={type(timestamp)}")
        logging.debug(f"🔍 [DEBUG] 调用栈:\n{''.join(traceback.format_stack()[-3:-1])}")
        
        if not hasattr(self, 'trader_engine') or self.trader_engine is None:
            logging.debug(f"🔍 [DEBUG] trader_engine 不存在或为 None，返回 None")
            return None
        
        try:
            # 🔍 调试断点：开始查询
            logging.debug(f"🔍 [DEBUG] 开始查询数据库: symbol={symbol}")
            
            # 🔧 修复：处理 timestamp 可能是字符串的情况
            if isinstance(timestamp, str):
                ...
                try:
                    ...
                    logging.error(f"🔍 [DEBUG] 无法解析时间戳字符串 '{timestamp}': {e}")
                    return None
            elif hasattr(timestamp, 'timestamp'):
                # 已经是 datetime 对象，无需转换
                pass
            else:
                # 尝试使用 pandas 转换
                try:
                    ...
                    if pd.isna(timestamp):
                        logging.error(f"🔍 [DEBUG] 时间戳转换失败: {timestamp}")
                        return None
                    timestamp = timestamp.to_pydatetime() if hasattr(timestamp, 'to_pydatetime') else timestamp
                except Exception as e:
                    logging.error(f"🔍 [DEBUG] 时间戳转换失败: {e}")
                    return None
            ...
            logging.debug(f"🔍 [DEBUG] 查询参数: target_ts={target_ts}, start_ts={start_ts}, end_ts={end_ts}")
            
            # PostgreSQL 使用参数化查询（命名参数）
            query = text("""
            ...
            # 🔍 调试断点：执行查询前
            logging.debug(f"🔍 [DEBUG] 准备执行数据库查询")
            
            with self.trader_engine.connect() as conn:
                logging.debug(f"🔍 [DEBUG] 数据库连接已建立")
                result = conn.execute(
                    query,
                    {
                        'symbol': symbol,
                ...
                logging.debug(f"🔍 [DEBUG] 查询已执行，开始获取结果")
                row = result.fetchone()
                logging.debug(f"🔍 [DEBUG] 查询结果: row={row}")
                ...
                if row:
                    ...
                    logging.debug(f"🔍 [DEBUG] 返回数据: {result_dict}")
                    return result_dict
            
            logging.debug(f"🔍 [DEBUG] 未找到数据，返回 None")
            return None
            ...
            logging.error(f"🔍 [DEBUG] 获取顶级交易者数据失败 {symbol}: {e}")
            logging.error(f"🔍 [DEBUG] 异常堆栈:\n{error_trace}")
            return None
    ...
    def check_trader_signal_filter(self, symbol: str, signal_datetime: datetime) -> tuple:
        ...
        try:
            logging.debug(f"🔍 [DEBUG] check_trader_filter 调用 get_top_trader_account_ratio: symbol={symbol}")
            trader_data = self.get_top_trader_account_ratio(symbol, signal_datetime)
            logging.debug(f"🔍 [DEBUG] check_trader_filter 获取到数据: trader_data={trader_data}")
    ...
    def check_trader_stop_loss(self, position: Dict, current_datetime: datetime) -> tuple:
        ...
        try:
            ...
            if entry_account_ratio is None:
                ...
                if entry_datetime:
                    logging.debug(f"🔍 [DEBUG] check_trader_stop_loss 调用 get_top_trader_account_ratio (entry): symbol={symbol}")
            ...
            # 获取当前的账户多空比
            logging.debug(f"🔍 [DEBUG] check_trader_stop_loss 调用 get_top_trader_account_ratio (current): symbol={symbol}")
    ...
    def check_signal_surge(self, symbol: str, signal_date: str, signal_close: float) -> tuple:
        ...
        try:
            ...
            prev_1h_close = result[0] if result else None
            if prev_1h_close is None:
                return True, 0.0
        ...
        try:
            ...
            if current_datetime >= window_12h_end:
                ...
                try:
                    ...
                    """)
                    with engine.connect() as conn:
                        result = conn.execute(query, {"window_start_ts": window_start_ts, "window_end_ts": window_end_ts}).fetchone()
    ...
    def get_all_symbols(self) -> List[str]:
        ...
        if self._all_symbols_cache is not None:
            logging.debug(f"使用缓存的交易对列表，共 {len(self._all_symbols_cache)} 个")
            return self._all_symbols_cache
        
        logging.info("🔍 开始查询数据库获取所有USDT交易对...")
        import time
        ...
        try:
            ...
            with engine.connect() as conn:
                logging.info("✅ 数据库连接成功，执行查询...")
                result = conn.execute(query)
                tables = [row[0] for row in result.fetchall()]
                logging.info(f"📊 查询完成，找到 {len(tables)} 个表")
            
            symbols = [
                table_name.replace('K1d', '') 
                for table_name in tables 
                if table_name.replace('K1d', '').endswith('USDT')
            ]
            self._all_symbols_cache = symbols
            query_duration = time.time() - query_start
            logging.info(f"✅ 找到 {len(symbols)} 个USDT交易对（耗时 {query_duration:.2f}秒）")
            return symbols
        except Exception as e:
    ...
    def get_daily_1hour_surge_signals(self, check_date: str) -> List[Dict]:
        ...
        try:
            ...
            processed_count = 0
            last_progress_time = scan_start
            
            for idx, symbol in enumerate(all_symbols, 1):
                # 每处理100个交易对或每10秒输出一次进度
                current_time = time.time()
                if idx % 100 == 0 or (current_time - last_progress_time) >= 10:
                    elapsed = current_time - scan_start
                    progress_pct = (idx * 100 // total_symbols) if total_symbols > 0 else 0
                    logging.info(f"  扫描进度: {idx}/{total_symbols} ({progress_pct}%) | 已找到信号: {len(signals)} 个 | 耗时: {elapsed:.1f}秒")
                ...
                try:
                    ...
                    with engine.connect() as conn:
                        yesterday_row = conn.execute(yesterday_query, {
                    ...
                    today_query = text(f'''
                        ...
                        FROM "{hourly_table}"
                        WHERE trade_date LIKE :check_pattern
                        ORDER BY trade_date ASC
                    ''')
                    
                    with engine.connect() as conn:
                        result = conn.execute(today_query, {"check_pattern": f'{check_date}%'})
                        today_hours = result.fetchall()
                    if not today_hours:
                        continue
                    ...
                    logging.debug(f"处理 {symbol} 时出错: {e}")
                    continue
                finally:
                    processed_count += 1
    ...
    def execute_trade(self, symbol: str, entry_price: float, entry_date: str, 
        ...
        try:
            ...
            if self.pending_virtual_compensations > 0:
                # 🔧 修复：限制补偿倍数上限为2倍
                # 最多累积1次补偿：1 + (2.0-1)*1 = 2倍
                effective_compensations = min(self.pending_virtual_compensations, 1)  # 🔧 改为最多累积1次（2倍上限）
            ...
            if hasattr(self, 'enable_trader_filter') and self.enable_trader_filter:
                logging.debug(f"🔍 [DEBUG] 建仓时调用 get_top_trader_account_ratio: symbol={symbol}")
                    ...
                    if self.enable_weak_24h_exit and entry_hour_timestamp and position.get('status') == 'normal':
                        ...
                        if 24 <= hours_held < 25:
                            ...
                            if return_24h < self.weak_24h_threshold:
                                ...
                                # 返还资金（释放资金，但保留仓位槽）
                                # 🔧 关键修复：如果仓位已经通过虚拟补仓返还过资金，就不要再返还了！
                                if position.get('capital_already_returned', False):
                                    # 虚拟补仓时已经返还过资金了，不要重复返还
                                    actual_invested = position.get('base_position_value', position['position_value'])
                                    weak_pnl = (current_price - position['avg_entry_price']) / position['avg_entry_price'] * self.leverage * actual_invested
                                    logging.info(f"💭 观察模式（虚拟补仓）: {symbol} 资金已返还过，不重复返还")
                                elif position.get('is_virtual_tracking', False):
                                    actual_invested = position.get('base_position_value', position['position_value'])
                                    self.capital += actual_invested
                                    weak_pnl = (current_price - position['avg_entry_price']) / position['avg_entry_price'] * self.leverage * actual_invested
                                    position['capital_already_returned'] = True  # 标记资金已返还
                                    logging.info(f"💭 观察模式（虚拟补仓）: 返还实际投入${actual_invested:.2f}")
                                else:
                                    ...
                                    position['capital_already_returned'] = True  # 标记资金已返还
                                    logging.info(f"💭 观察模式（正常仓位）: 返还资金${position['position_value']:.2f}")
    ...
    def add_position(self, position: Dict, current_price: float, current_date: str):
        ...
        try:
            if self.use_virtual_add_position:
                ...
                # 💡 使用未补偿的基础金额计算实际亏损
                # 问题：position_value可能已经包含补偿金额，导致real_pnl被放大
                # 解决：使用base_position_value（建仓时记录的未补偿基础金额）
                base_value = position.get('base_position_value', position_value)  # 未补偿的原始金额
                price_change_pct = (current_price - entry_price) / entry_price  # -18%
                loss_rate = price_change_pct * self.leverage  # -18% × 4 = -72%
                actual_loss = base_value * loss_rate  # ✅ 基于未补偿的原始金额计算亏损
                
                
                #🔧 关键修复：首次虚拟补仓时返还首仓本金
                if 'virtual_add_count' not in position:
                    position['virtual_add_count'] = 0
                    # 首次虚拟补仓：返还原始首仓本金（使用position_value，即实际投入金额）
                    self.capital += position_value
                    ...
                    logging.info(
                        ...
                        f"返还首仓本金:${position_value:.2f} 资金:${self.capital:.2f}"
                    )
                else:
                    logging.info(
                ...
                # ⚠️ 关键：虚拟补仓不实际扣除资金（这是"虚拟"的核心）
                # 不执行：self.capital -= virtual_add_value
                # 效果：资金池保持充足，后续建仓时有足够的资金
                
                logging.info(
                    f"💭 虚拟补仓: {position['symbol']} "
                    f"虚拟投入:${virtual_add_value:.2f}（不实际扣资金） "
                    f"当前资金:${self.capital:.2f}"
                )
                ...
                # Step 4: 更新持仓信息
                # 🔧 修复BUG：更新position_value以正确显示总投入金额
                # 虚拟补仓虽然不实际扣资金，但应该"虚拟占用"，以便CSV报告显示正确的仓位金额
                position['position_value'] += virtual_add_value  # ✅ 累加虚拟补仓金额（与实际补仓保持一致）
                ...
                position['real_exit_price'] = current_price  # 记录虚拟补仓时的价格（用于报告）
                position['real_pnl'] = actual_loss  # 💡 只记录首仓实际亏损（基于原始投入）
    ...
    def exit_position(self, position: Dict, exit_price: float, exit_date: str, exit_reason: str):
        ...
        try:
            ...
            if position.get('is_virtual_tracking', False):
                ...
                # 💰 虚拟平仓时不返还资金
                # 原因：虚拟补仓时首仓本金已返还，虚拟补仓金额未实际扣除
                # 所以虚拟平仓时不需要返还任何资金
                # ⚠️ 不执行：self.capital += position_value
                ...
                logging.info(
                    ...
                    f"实际PnL:${position.get('real_pnl', 0):.2f} 原因:{exit_reason} "
                    f"资金:${self.capital:.2f} ✅ 释放槽位 "
                    f"📊 待补偿虚拟补仓: {self.pending_virtual_compensations}次"
                )
                return
            ...
            if position.get('capital_already_returned', False):
                ...
                # 虚拟补仓的情况：只返还实际投入的首仓本金
                actual_invested = position.get('base_position_value', position['position_value'])
                self.capital += actual_invested + pnl
                logging.info(f"💭 虚拟补仓平仓: {exit_reason} 实际投入${actual_invested:.2f} 显示仓位${position['position_value']:.2f} 盈亏${pnl:.2f} 资金:{self.capital:.2f}")
            else:
                # 正常情况：返还全部本金
                position_value = position['position_value']
    ...
    def get_entry_price(self, symbol: str, date_str: str) -> Optional[float]:
        ...
        try:
            ...
            with engine.connect() as conn:
                result = conn.execute(query, {"date_str": date_str, "date_pattern": f'{date_str}%'}).fetchone()
    ...
    def get_5m_closes_in_window(self, symbol: str, start_dt: datetime, end_dt: datetime) -> List[float]:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                result = conn.execute(query, {"start_str": start_str, "end_str": end_str})
    ...
    def run_backtest(self, start_date: str, end_date: str):
        ...
        logging.info("=" * 80)
        logging.info("🚀 开始买量暴涨策略回测（小时线优化版）")
        logging.info("=" * 80)
        logging.info(f"📅 回测日期范围: {start_date} 到 {end_date}")
        ...
        logging.info(f"⏱️  最大持仓时间: {self.max_hold_hours:.0f}小时（{self.max_hold_hours/24:.0f}天）")
        logging.info(f"🔍 顶级交易者过滤: {'启用' if self.enable_trader_filter else '禁用'}")
        ...
        logging.info(f"📝 解析日期: 开始={start_date_only}, 结束={end_date_only}")
        ...
        logging.info(f"✅ 日期解析完成，将处理 {total_days} 天")
        
        logging.info("🔄 开始主回测循环...")
        try:
            day_count = 0
            while current_date <= end_dt:
                ...
                # 🆕 每天输出进度信息
                logging.info(f"\n{'='*60}")
                logging.info(f"📅 [{day_count}] 处理日期: {date_str} | 当前资金: ${self.capital:,.2f} | 持仓数: {len(self.positions)} | 待建仓信号: {len(self.pending_signals)}")
                logging.info(f"{'='*60}")
                ...
                if len(self.positions) < self.max_daily_positions:
                    ...
                    logging.info(f"🔍 开始扫描 {date_str} 的买量暴涨信号...")
                    try:
                        daily_signals = self.get_daily_1hour_surge_signals(date_str)
                        scan_duration = time.time() - scan_start_time
                        logging.info(f"✅ 扫描完成，耗时 {scan_duration:.2f}秒，发现 {len(daily_signals)} 个信号")
                    except Exception as e:
                        scan_duration = time.time() - scan_start_time
                        logging.error(f"❌ 扫描 {date_str} 信号失败（耗时 {scan_duration:.2f}秒）: {e}")
                        import traceback
                        logging.error(traceback.format_exc())
                        daily_signals = []
                    ...
                    for signal in daily_signals:
                        ...
                        try:
                            self.signal_records.append({
                                ...
                                'entry_price': '',
                                'note': '',
                                'account_ratio': float(account_ratio) if account_ratio else None  # 🆕 保存账户多空比
                            })
                        except Exception:
                            pass
        ...
        for position in self.positions.copy():
            try:
                ...
                with engine.connect() as conn:
                    result = conn.execute(query, {"end_date": end_date, "end_pattern": f'{end_date}%'}).fetchone()
    ...
    def generate_trade_csv_report(self):
        ...
        # 创建输出目录：data/backtrade_records（在项目根目录下）
        # __file__ 是 backend/hm20260121.py，所以需要向上两级到项目根目录
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        output_dir = os.path.join(project_root, 'data', 'backtrade_records')
        os.makedirs(output_dir, exist_ok=True)
        
        csv_filename = os.path.join(output_dir, f"buy_surge_backtest_report_{timestamp}.csv")
    ...
    def generate_signal_csv_report(self):
        ...
        csv_filename = os.path.join(output_dir, f"buy_surge_signal_feedback_{timestamp}.csv")
        try:
