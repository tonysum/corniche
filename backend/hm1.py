...
class BuySurgeBacktest:
    ...
    def check_exit_conditions(self, position: Dict, current_price: float, current_date: str) -> bool:
        ...
        try:
            ...
            # 🔧 关键修复：确保 entry_datetime 和 current_datetime 都是 pandas Timestamp 类型
            ...
            # 🔧 关键修复：只看建仓时刻及之后的数据（严格使用 >= 和 <=）
            mask = (hourly_df['trade_datetime'] >= entry_datetime) & (hourly_df['trade_datetime'] <= current_datetime)
            ...
            if not hold_period_data.empty:
                ...
                if first_row_time < entry_datetime:
                    ...
                    hold_period_data = hourly_df[mask].copy().sort_values('trade_datetime')
            ...
            if not hold_period_data.empty:
                ...
                for idx, row in rows_to_check.iterrows():
                    ...
                    hour_datetime_str = hour_datetime_ts.strftime('%Y-%m-%d %H:%M:%S')  # 🆕 完整的日期时间字符串
        ...
        while current_date <= end_dt:
            ...
            for signal in self.pending_signals[:]:  # 复制列表避免迭代时修改
                ...
                if current_date > signal['timeout_datetime']:
                    logging.info(f"⏰ [{date_str}] {symbol} 信号超时，放弃建仓（买量{buy_surge_ratio:.1f}倍）")
                ...
                if not hourly_df.empty:
                    # 筛选信号日之后到当前日期的小时数据
                    hourly_df['trade_datetime'] = pd.to_datetime(hourly_df['trade_date'])
                    signal_datetime = pd.to_datetime(signal['signal_datetime'])
                    # 🔧 修复：将 current_date 转换为 pandas Timestamp 进行比较
                    current_datetime_for_check = pd.to_datetime(current_date) + pd.Timedelta(hours=23, minutes=59, seconds=59)
                    mask = (hourly_df['trade_datetime'] >= signal_datetime) & (hourly_df['trade_datetime'] <= current_datetime_for_check)
                    check_period_data = hourly_df[mask]
                    
                    # 检查是否有小时低点达到目标价格
                    # 🔧 性能优化：只检查最近的小时数据，避免遍历过多历史数据
                    max_hours_to_check_signal = 500  # 最多检查500小时（约21天）
                    signal_rows_to_check = check_period_data.tail(max_hours_to_check_signal) if len(check_period_data) > max_hours_to_check_signal else check_period_data
                    
                    for _, row in signal_rows_to_check.iterrows():
