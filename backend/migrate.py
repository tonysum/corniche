...
class SQLiteToPostgreSQLMigrator:
    ...
    def sqlite_type_to_postgresql(self, sqlite_type: str) -> str:
            ...
            with self.pg_engine.connect() as conn:
                ...
                try:
                    ...
                    for col in schema['columns']:
                        pg_type = self.sqlite_type_to_postgresql(col['type'])
    ...
    def migrate_all(self, table_filter: Optional[str] = None, skip_existing: bool = True) -> Dict:
        """迁移所有表"""
        # 获取所有表
...
当前配置（从 .env 或默认值）：
  ...
  PostgreSQL 密码: {'已设置' if pg_password else '未设置（需要提供）'}
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='表名过滤（例如: K1d 只迁移日线表，K5m 只迁移5分钟表）'
    )
    parser.add_argument(
        '--batch-size',
        type=int,
