#!/usr/bin/env python3
"""
买量暴涨策略回测程序 - 5分钟优化版（100-200倍和300-350倍区间）
基于5分钟主动买量暴涨（相对昨日平均5分钟买量）信号的做多策略，只保留100-200倍和300-350倍优质区间

策略逻辑：
1. 扫描所有USDT交易对，使用5分钟K线数据检测买量暴涨信号（只保留100-200倍和300-350倍优质区间）
...
5. 止盈：基于建仓价10.5%（动态可提升至18-19%）
6. 止损：补仓后基于新平均成本下跌18%
7. 最大持仓天数：3天
8. 超时机制：信号后48小时内未达到目标跌幅则放弃
...
class BuySurgeBacktest:
    ...
    def __init__(self):
        ...
        self.buy_surge_threshold = 100  # 5分钟买量相对昨日平均5分钟买量的暴涨阈值下限（100倍）
        self.buy_surge_threshold_max = 350  # 5分钟买量暴涨阈值上限（350倍，实际只保留100-200和300-350区间）
        self.take_profit_pct = 0.095  # 止盈比例 (10.5%)
        self.add_position_trigger_pct = -0.15  # 补仓触发比例 (-18%)
        self.stop_loss_pct = -0.18  # 止损比例 (-18%，补仓后基于新平均成本)
        self.max_hold_hours = 72  # 最大持仓小时数 (72小时/3天强制平仓)
        self.max_positions = 5  # 最大同时持仓数量
        ...
        self.wait_drop_pct_config = [
            (50, -0.09),     # <50倍：等待9%回调
            (200, -0.09),    # 50-200倍：等待9%回调
            (300, -0.05),    # 200-300倍：等待3%回调
            (9999, -0.05),   # 300倍以上：等待2%回调
        ]
    ...
    def calculate_dynamic_take_profit(self, position: Dict, hourly_df: pd.DataFrame, entry_datetime: datetime) -> float:
        ...
        Returns:
            动态止盈阈值（如0.15表示15%，0.18表示18%，0.19表示19%）
        """
        try:
            # 获取建仓价格
            ...
            one_hour_later_ts = entry_ts + 3600 * 1000
            
            data_1h = hourly_df[
                (hourly_df['open_time'] >= entry_ts) & 
                (hourly_df['open_time'] < one_hour_later_ts)
            ]
            
            # 如果数据不足30分钟（6个周期），使用默认止盈
            if len(data_1h) < 6:
                return self.take_profit_pct
            
            # 计算1小时内的最大涨幅（基于最高价）
            max_high = data_1h['high'].max()
            max_gain_pct = (max_high - avg_price) / avg_price
            
            # 根据最大涨幅动态调整止盈阈值
            if max_gain_pct >= 0.22:  # 1小时内最大涨幅≥22%
                adjusted_tp = 0.19  # 止盈设为19%
                logging.info(f"🚀 {position['symbol']} 超强势币，1小时内最大涨幅{max_gain_pct*100:.1f}%，止盈提高到{adjusted_tp*100:.0f}%")
                return adjusted_tp
            elif max_gain_pct >= 0.20:  # 1小时内最大涨幅≥20%
                adjusted_tp = 0.18  # 止盈设为18%
                logging.info(f"📈 {position['symbol']} 强势币，1小时内最大涨幅{max_gain_pct*100:.1f}%，止盈提高到{adjusted_tp*100:.0f}%")
                return adjusted_tp
            else:
                return self.take_profit_pct  # 保持基础15%
            ...
            for table_row in daily_tables:
                ...
                try:
                    ...
                    for period_data in today_5m_periods:
                        ...
                        # 满足阈值，记录信号（只保留100-200倍和300-350倍区间）
                        if threshold <= surge_ratio <= self.buy_surge_threshold_max:
                            # 🎯 倍数区间过滤：只保留表现优秀的区间
                            # 100-200倍：包含100-120(优秀)和120-200(一般)
                            # 300-350倍：高收益区间
                            if not ((100 <= surge_ratio <= 200) or (300 <= surge_ratio <= 350)):
                                continue  # 过滤掉其他区间
    ...
    def _smart_check_hour(self, position: Dict, hour_5m_df: pd.DataFrame, dynamic_tp_pct: float, 
        ...
        # 🚀 情况2：只可能止损（快）
            ...
            if not loss_bars.empty:
                ...
                logging.info(f"🛑 止损(快速): {symbol} 低{exit_price:.6f}")
                return True
        ...
        # 🚀 情况3：只可能补仓（快）
            ...
            if not add_bars.empty:
                ...
                logging.info(f"🔄 补仓(快速): {symbol} 低{add_price:.6f}")
                ...
                if not loss_after_add.empty:
                    ...
                    self.exit_position(position, exit_price, exit_datetime, "stop_loss")
                    logging.info(f"🛑 补仓后止损: {symbol} 低{exit_price:.6f}")
                    return True
        ...
        # 🐌 情况4：多个事件可能冲突，必须逐根检查
            ...
            for idx, row in hour_5m_df.iterrows():
                ...
                if profit_pct >= dynamic_tp_pct:
                    ...
                    self.exit_position(position, high_price, period_datetime_str, "take_profit")
                    logging.info(f"💰 止盈(逐根): {symbol} 高{high_price:.6f}")
                    return True
                ...
                if not position.get('has_add_position', False):
                    ...
                    if add_trigger_pct <= self.add_position_trigger_pct:
                        ...
                        drawdown_at_add = (current_avg_price - low_price) / current_avg_price
                        if drawdown_at_add > position.get('max_drawdown', 0):
                            position['max_drawdown'] = drawdown_at_add
                        
                        self.add_position(position, low_price, period_date)
                        logging.info(f"🔄 补仓(逐根): {symbol} 低{low_price:.6f}")
                        ...
                        if profit_after_add >= dynamic_tp_pct:
                            ...
                            self.exit_position(position, high_price, period_datetime_str, "take_profit")
                            logging.info(f"✨ 补仓后止盈(同K线): {symbol}")
                            return True
                        
                        loss_after_add = (low_price - new_avg_price) / new_avg_price
                        if loss_after_add <= self.stop_loss_pct:
                            # 🔧 修复：止损前更新最大跌幅
                            drawdown_at_loss = (new_avg_price - low_price) / new_avg_price
                            if drawdown_at_loss > position.get('max_drawdown', 0):
                                position['max_drawdown'] = drawdown_at_loss
                            
                            self.exit_position(position, low_price, period_datetime_str, "stop_loss")
                            logging.info(f"🛑 补仓后止损(同K线): {symbol}")
                            return True
                ...
                if position.get('has_add_position', False):
                    ...
                    if loss_pct <= self.stop_loss_pct:
                        ...
                        drawdown_at_stop = (current_avg_price - low_price) / current_avg_price
                        if drawdown_at_stop > position.get('max_drawdown', 0):
                            position['max_drawdown'] = drawdown_at_stop
                        
                        self.exit_position(position, low_price, period_datetime_str, "stop_loss")
                        logging.info(f"🛑 止损(逐根): {symbol} 低{low_price:.6f}")
                        return True
    ...
    def run_backtest(self, start_date: str, end_date: str):
        """运行回测"""
        logging.info(f"开始买量暴涨策略回测（5分钟优化版）: {start_date} 到 {end_date}")
        logging.info(f"初始资金: ${self.initial_capital:,.2f}")
        logging.info(f"杠杆倍数: {self.leverage}x")
        logging.info(f"买量暴涨阈值: 100-200倍 + 300-350倍（只保留优质区间，某5分钟买量 vs 昨日平均5分钟买量）")
        logging.info(f"等待策略: 100-200倍→9%, 300-350倍→2%")
        logging.info(f"最大持仓时间: {self.max_hold_hours:.0f}小时（{self.max_hold_hours/24:.0f}天）")
        logging.info(f"⚡ 优势：5分钟级别信号检测、建仓和止盈止损（响应速度提升12倍）")
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=100.0,
        help='买量暴涨阈值下限(默认: 100.0倍, 只保留100-200倍和300-350倍优质区间 - 5分钟买量 vs 昨日平均5分钟买量)'
    )
    ...
    if args.threshold:
        ...
        logging.info(f"买量暴涨阈值设置为: {args.threshold}倍起，只保留100-200倍和300-350倍区间")
