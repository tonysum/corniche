...
【1️⃣ 信号发现与建仓】
  ...
  📡 信号扫描逻辑（get_hourly_buy_surge_signals函数）：
    ...
    • 触发条件：小时主动买量 >= 昨日24小时平均买量 × 2.2倍
    • 倍数限制：默认仅接受2.2-2.9倍信号（可通过--max-multiple参数放宽）
      - <2.2倍：正常波动，不触发
      - 2.2-2.9倍：稳健信号，默认接受 ✅
      - >=3.0倍：高波动，默认过滤（可放宽）
  ...
  🎁 虚拟补仓补偿机制：
    • 触发条件：上一笔交易发生虚拟补仓平仓后
    • 补偿系数：1.05倍（virtual_add_compensation_multiplier）
    • 补偿次数：最多累积1次（最高1.05倍建仓金额）
    • 上限保护：单笔建仓不超过总资金30%
    • 消耗机制：建仓成功后消耗1次补偿机会
    • 设计理念：用下次交易的适度增加仓位（+5%）来补偿虚拟补仓的损失（更保守）
...
# 数据库路径
# CRYPTO_DB_PATH 已移除，使用 PostgreSQL
...
class BuySurgeBacktest:
    ...
    def __init__(self):
        ...
        # 🆕 添加：顶级交易者风控参数
        self.enable_trader_filter = False #True  # 是否启用顶级交易者信号筛选（默认开启）
        self.min_account_ratio = 0.70  # 最小账户多空比（信号筛选）
        self.account_ratio_stop_threshold = -9999  # 账户多空比下降止损阈值

        # 回测参数
        self.initial_capital = 10000.0  # 初始资金
        self.leverage = 4.0  # 杠杆倍数（4倍）
        self.position_size_ratio = 0.06  # 单次建仓占资金比例（6%）
        self.add_position_size_ratio = 0.05  # 补仓占资金比例（5%，可以设置为首仓的倍数）
        self.max_daily_positions = 6  # 并发持仓上限（保守设置，留出缓冲空间）
        self.buy_surge_threshold = 2.2  # 小时主动买量比昨日暴涨阈值（2.2倍）
        self.buy_surge_max = 3.0  # 买量暴涨倍数上限（保留2.2-2.9倍信号，2.9倍信号贡献大量收益）
        ...
        # 🆕 BTC风控参数
        self.enable_btc_crash_filter = False  # 是否启用BTC暴跌风控（已禁用）
        self.btc_crash_threshold = -0.03  # BTC 1小时跌幅阈值（-3%）
        self.btc_crash_delay_hours = 1  # BTC暴跌后延迟建仓时间（1小时）
        ...
        # 🆕 虚拟补仓补偿机制
        self.virtual_add_compensation_multiplier = 1.05  # 虚拟补仓后，下次建仓投入金额的补偿系数（1.05倍，更保守的补偿）
        ...
        logging.info(f"   virtual_add_compensation_multiplier = {self.virtual_add_compensation_multiplier} (应该是1.05)")
    ...
    def __del__(self):
        """析构函数，确保数据库连接关闭"""
        # PostgreSQL 连接由 engine 管理，无需手动关闭
        pass
        ...
        try:
            ...
            if current_datetime >= window_2h_end:
                ...
                try:
                    ...
                    with self.engine.connect() as conn:
                        closes = [row[0] for row in conn.execute(query, {"start_ts": start_ts, "end_ts": end_ts}).fetchall()]
    ...
    def get_daily_buy_surge_coins(self, date_str: str) -> List[Dict]:
        ...
        try:
            with self.engine.connect() as conn:
                # 获取所有交易对
                query_tables = text("SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name LIKE 'K1d%USDT'")
                ...
                for table_row in tables:
                    ...
                    try:
                        ...
                        today_result = conn.execute(query_today, {
                            "date_str": date_str,
                        ...
                        yesterday_result = conn.execute(query_yesterday, {
                            "yesterday_str": yesterday_str,
                            "yesterday_like": f'{yesterday_str}%'
                        }).fetchone()
    ...
    def get_daily_1hour_surge_signals(self, check_date: str) -> List[Dict]:
        ...
        try:
            ...
            signals = []
            threshold = 2.2  # 🔥 某小时买量 >= 昨日平均小时买量 × 2.2倍
            ...
            with self.engine.connect() as conn:
                for idx, symbol in enumerate(all_symbols, 1):
                    try:
                        ...
                        yesterday_row = conn.execute(query_daily, {
                            "yesterday_date": yesterday_date,
                            "yesterday_like": f'{yesterday_date}%'
                        }).fetchone()
                        ...
                        today_hours = conn.execute(query_hourly, {
                            "check_date_like": f'{check_date}%'
                        }).fetchall()
                        ...
                        for hour_data in today_hours:
                            ...
                            # 满足阈值，记录信号（只保留2-2.9倍的稳健信号，严格排除>=3倍）
    ...
    def get_btc_1h_change(self, check_datetime) -> float:
        """获取BTC在指定时间的1小时跌幅
        
        Args:
            check_datetime: 检查的时间点（datetime对象或字符串）
            
        Returns:
            BTC 1小时跌幅百分比（如-5.2表示跌5.2%）
        """
        try:
            # 确保是datetime对象
            ...
            # 获取当前时间前后1天的BTC小时K线数据
            start_date = (check_datetime - pd.Timedelta(days=1)).strftime('%Y-%m-%d')
            end_date = (check_datetime + pd.Timedelta(days=1)).strftime('%Y-%m-%d')
            
            btc_df = self.get_hourly_kline_data('BTCUSDT', start_date=start_date, end_date=end_date)
            
            if btc_df.empty:
                logging.warning(f"⚠️ 无法获取BTC数据，跳过BTC风控检查")
                return 0.0  # 无数据时返回0，不触发风控
            
            # 转换时间和价格
            btc_df['trade_datetime'] = pd.to_datetime(btc_df['trade_date'])
            btc_df['close'] = btc_df['close'].astype(float)
            
            # 找到当前时间点的BTC价格
            current_btc = btc_df[btc_df['trade_datetime'] <= check_datetime].tail(1)
            if len(current_btc) == 0:
                return 0.0
            
            current_price = current_btc.iloc[0]['close']
            current_time = current_btc.iloc[0]['trade_datetime']
            
            # 找到1小时前的BTC价格
            time_1h_ago = current_time - pd.Timedelta(hours=1)
            btc_1h_ago = btc_df[btc_df['trade_datetime'] <= time_1h_ago].tail(1)
            
            if len(btc_1h_ago) == 0:
                return 0.0
            
            price_1h_ago = btc_1h_ago.iloc[0]['close']
            
            # 计算1小时跌幅
            btc_change = ((current_price - price_1h_ago) / price_1h_ago) * 100
            
            return btc_change
            
        except Exception as e:
            logging.warning(f"⚠️ 获取BTC 1小时跌幅失败: {e}")
            return 0.0  # 出错时返回0，不触发风控
    ...
    def run_backtest(self, start_date: str, end_date: str):
        ...
        while current_date <= end_dt:
            ...
            # 检查待建仓信号（使用小时线数据）
            # 🔥 优先级排序：买量倍数越高越优先（高倍数信号盈亏比更好）
            sorted_signals = sorted(self.pending_signals[:], key=lambda x: x['buy_surge_ratio'], reverse=True)
            
            signals_to_remove = []
            for signal in sorted_signals:  # 使用排序后的信号列表
                ...
                if not hourly_df.empty:
                    ...
                    for _, row in check_period_data.iterrows():
                        if row['low'] <= target_price:
                            ...
                            # 🆕 BTC风控：检查BTC 1小时跌幅
                            if self.enable_btc_crash_filter:
                                btc_1h_change = self.get_btc_1h_change(entry_datetime)
                                
                                if btc_1h_change < self.btc_crash_threshold:
                                    # BTC暴跌，检查是否已过延迟期
                                    delay_hours = (entry_datetime - signal_datetime).total_seconds() / 3600
                                    
                                    if delay_hours < self.btc_crash_delay_hours:
                                        # 还在延迟期内，暂不建仓
                                        logging.info(f"🚫 {symbol} BTC暴跌风控：BTC 1h跌幅{btc_1h_change:+.2f}%，"
                                                    f"已等待{delay_hours:.1f}h < {self.btc_crash_delay_hours}h，暂不建仓")
                                        continue  # 继续检查下一个时间点
                                    else:
                                        # 已过延迟期，可以建仓
                                        logging.info(f"⚠️ {symbol} BTC暴跌已过延迟期：BTC 1h跌幅{btc_1h_change:+.2f}%，"
                                                    f"已等待{delay_hours:.1f}h > {self.btc_crash_delay_hours}h，允许建仓")
                                else:
                                    # BTC正常/上涨，无风控限制
                                    if abs(btc_1h_change) > 1:  # 只在变化较大时记录
                                        logging.debug(f"✅ {symbol} BTC风控通过：BTC 1h变化{btc_1h_change:+.2f}%")
    ...
    def generate_trade_csv_report(self):
        ...
        filename = f"buy_surge_backtest_report_{timestamp}.csv"
        
        # 从环境变量获取输出路径
        output_dir = os.getenv('CSV_OUTPUT_PATH', '.')
        
        # 如果是相对路径，且不是当前目录，则尝试基于项目根目录解析
        if output_dir and output_dir != '.' and not os.path.isabs(output_dir):
            # 获取当前脚本所在目录 (backend)
        ...
        if output_dir and output_dir != '.':
            try:
                ...
                logging.warning(f"无法创建目录 {output_dir}: {e}，将保存到当前目录")
                output_dir = '.'
            ...
            with open(csv_filename, 'w', newline='', encoding='utf-8-sig') as csvfile:
                ...
                for i, trade in enumerate(entered_trades, 1):
                    ...
                    row = {
                        ...
                        # 🔧 只有真实补仓才显示"✅是"，虚拟补仓显示"否"（虽然has_add_position=True）
                        '是否有补仓': (
                            '✅是' if trade.get('has_add_position', False) and not trade.get('is_virtual_tracking', False)
                            else '否'
                        ),
    ...
    def generate_signal_csv_report(self):
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f"buy_surge_signal_feedback_{timestamp}.csv"
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help='买量暴涨倍数上限（默认3.0；保留2.2-2.9倍信号，只做稳健区间）'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='最近N天回测 (可选, 覆盖 start-time/end-time)'
    )
    ...
    if args.days is not None:
        ...
        # 将日期格式化为 YYYY-MM-DD
        args.start_time = start_date.strftime('%Y-%m-%d')
        args.end_time = end_date.strftime('%Y-%m-%d')
        logging.info(f"按天数回测: 最近 {args.days} 天 ({args.start_time} 至 {args.end_time})")
